...
let page='home';
const navItems=[['home','⌂','Home'],['todos','✓','To Do'],['notes','📝','Notities'],['mail','✉️','Mail'],['class','👥','Klas'],['reflection','💭','Reflectie']];
const loaders={
...
function markNav(){const active=['groups','wheel','soundboards','turns','timer','feedback','points'].includes(page)?'class':page;app.querySelectorAll('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===active));const sb=app.querySelector('#globalSettings');if(sb)sb.classList.toggle('active',page==='settings');}
function renderClass(root){root.innerHTML=`<div class="topbar"><div><div class="title">Klas</div><div class="subtitle">Hulpmiddelen voor dagelijks gebruik in de klas.</div></div></div><div class="grid"><div class="tile" data-open="groups"><div class="icon">👥</div><div><strong>Groepjesmaker</strong><div class="muted tile-sub">Maak en pas groepjes aan.</div></div></div><div class="tile" data-open="wheel"><div class="icon">🎯</div><div><strong>Namenrad</strong><div class="muted tile-sub">Draai een willekeurige leerling.</div></div></div><div class="tile" data-open="soundboards"><div class="icon">🔊</div><div><strong>Soundboards</strong><div class="muted tile-sub">MP3, M4A, WAV en andere audiobestanden.</div></div></div>
    <div class="tile" data-open="turns"><div class="icon">🙋</div><div><strong>Beurten</strong><div class="muted tile-sub">Kies leerlingen willekeurig en houd bij wie al geweest is.</div></div></div>
    <div class="tile" data-open="timer"><div class="icon">⏱️</div><div><strong>Timer</strong><div class="muted tile-sub">Grote klassikale timer met snelle tijden.</div></div></div>
    <div class="tile" data-open="feedback"><div class="icon">💬</div><div><strong>Feedback</strong><div class="muted tile-sub">Leg feedback vast met leerlingkeuze, foto, spraak en AI-samenvatting.</div></div></div>
    <div class="tile" data-open="points"><div class="icon">🏆</div><div><strong>Punten</strong><div class="muted tile-sub">Geef per leerling punten erbij of eraf en mail het overzicht.</div></div></div>
</div>`;root.querySelectorAll('[data-open]').forEach(t=>t.onclick=()=>navigate(t.dataset.open));}
...
if('serviceWorker' in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./service-worker.js?v=15');await reg.update?.()}catch(e){console.warn(e)}});
