...
let page='home';
const navItems=[['home','⌂','Home'],['class','👥','Class'],['mail','✉️','Mail'],['agenda','📅','Agenda'],['todos','✓','To Do']];
const loaders={
...
function renderClass(root){
  ...
  <div class="grid">
    ...
    <div class="tile" data-open="absences"><div class="icon">📋</div><div><strong>Absenties invoeren</strong><div class="muted tile-sub">Geoorloofd of ongeoorloofd, dagdeel en optionele reden.</div></div></div>
    <div class="tile" data-open="reflection"><div class="icon">💭</div><div><strong>Reflectie</strong><div class="muted tile-sub">Les- en dagreflectie met AI-samenvatting.</div></div></div>
  </div>`;
...
if('serviceWorker' in navigator)window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./service-worker.js?v=37');await reg.update?.()}catch(e){console.warn(e)}});
